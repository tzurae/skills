...
3. **Branch: is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet Issues, each declaring its **blocking edges**. On a local tracker that is one file per Issue under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any Issue whose blockers are done can be grabbed: kick off **`/implement`** per Issue, **`/clear`ing context between each one**. Each Issue is self-contained, so the last one's context is disposable.
   - **No** → **`/implement`** right here, in the same context window.

   Either way, **`/implement`** establishes one Issue, branch, and dedicated worktree; drives **`/tdd`** internally; verifies the real runtime surface; and reviews the working tree on separate Standards and Spec axes. It then prepares the commit, push, evidence, and pull request and waits for explicit approval. It never merges. Reach for **`/tdd`** on its own when you just want to build one concrete behaviour test-first, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.

### Context hygiene

Keep steps 1–3 in **one unbroken context window** (don't compact or clear until after `/to-tickets`) so the grilling, spec, and Issues all build on the same thinking. Each `/implement` then starts fresh, working from one Issue.
